...
Comment.likeComment = async (comment, liker) => {
  const flag = await pool.query(
    `SELECT * FROM comments_like WHERE comment_id=? AND liker_id=?`,
    [comment, liker]
  );
  ...
  if (flag[0].length) {
    ...
    const result_a = await pool.query(
      `INSERT INTO comments_like (comment_id, liker_id) VALUES (?, ?)`,
      [comment, liker]
    );
    const result_b = await pool.query(
...
Comment.orderByTime = async () => {
  const [rows] = await pool.query(`SELECT comments.*, users.name 
    FROM comments 
    INNER JOIN users 
    ON comments.author = users.user_id ORDER BY comment_time DESC`);
  return rows;
};
...
Comment.orderByLike = async () => {
  const [rows] = await pool.query(
    ...
    FROM comments 
    INNER JOIN users 
    ON comments.author = users.user_id ORDER BY likes_count DESC`
  );
  return rows;
};
