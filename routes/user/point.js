...
//이부분 취약점이 있을 수 있음
//router.get("/signup", point.signupPoint, userGetPoint); //1 회원가입시 30000p 지급
router.get("/attend", isSignedIn, point.attendancePoint, userGetPoint); //2 출석시 20000p 지급(로그인 했을 때, 하루마다 초기화)
//router.get("/recommended", point.recommendedPoint, userGetPoint); //3 추천인으로 지목당했을 때 30000p 지급
//router.get("/recommender", point.recommenderPoint, userGetPoint); //4 추천인을 지목했을 때 20000p 지급
